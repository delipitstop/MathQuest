...
function geronimo() {
				...
				if ((field === "pill") || (field === "powerpill")) {
					...
					if (
						...
						if (field === "powerpill") {
							...
							// Show maths question!
							if (!window.mathOverlayActive) {
								var q = makeMathQuestion();
								window.showMathOverlay(q);
							}
						} else {
							Sound.play("waka");
							s = PILL_POINTS;
	...
	function animationLoop() {
		...
		if (game.pause !== true && !window.mathOverlayActive) {
...
function makeMathQuestion() {
    ...
    if (op === '+') {
        ...
        b = Math.floor(Math.random() * Math.min(a - 1, 50)) + 1;
        answer = a - b;
    } else if (op === '\u00D7') {
...
window.mathQuestion = null;
window.mathOverlayActive = false;
window.mathTimerId = null;

window.showMathOverlay = function(q) {
    window.mathQuestion = q;
    window.mathOverlayActive = true;
    window.mathActive = true;
    var overlay = document.getElementById('mathOverlayPacman');
    if (overlay) {
        overlay.style.display = 'flex';
        var qEl = document.getElementById('mathQ4');
        if (qEl) qEl.textContent = q.text + ' = ?';
        var inp = document.getElementById('mathInput4');
        if (inp) { inp.value = ''; inp.focus(); }
        var fb = document.getElementById('mathFeedback4');
        if (fb) fb.textContent = '';
    }
    if (window.mathTimerId) clearInterval(window.mathTimerId);
    var timeLeft = 15;
    var timerEl = document.getElementById('mathTimer4');
    window.mathTimerId = setInterval(function() {
        timeLeft--;
        if (timerEl) timerEl.textContent = timeLeft + 's';
        if (timeLeft <= 0) {
            clearInterval(window.mathTimerId);
            window.rejectMathAnswer();
        }
    }, 1000);
};

window.submitMathAnswerPacman = function() {
    if (!window.mathOverlayActive || !window.mathQuestion) return;
    var val = parseInt(document.getElementById('mathInput4').value);
    var fb = document.getElementById('mathFeedback4');
    if (val === window.mathQuestion.answer) {
        if (fb) { fb.textContent = '\u2705 Ghost Freeze! +100 pts'; fb.style.color = '#34D399'; }
        if (typeof game !== 'undefined') game.score.add(100);
        clearInterval(window.mathTimerId);
        // Keep beast mode active longer!
        if (typeof pacman !== 'undefined') {
            pacman.beastModeTime += 8000;
        }
        setTimeout(window.closeMathOverlayPacman, 1000);
        window.mathQuestion = null;
    } else {
...
window.rejectMathAnswer = function() {
    if (!window.mathOverlayActive) return;
    clearInterval(window.mathTimerId);
    var fb = document.getElementById('mathFeedback4');
    if (fb) fb.textContent = '\u23F3 Time\'s up! Ghosts resume...';
    window.mathQuestion = null;
    setTimeout(window.closeMathOverlayPacman, 800);
};

window.closeMathOverlayPacman = function() {
    window.mathOverlayActive = false;
    window.mathActive = false;
    var overlay = document.getElementById('mathOverlayPacman');
